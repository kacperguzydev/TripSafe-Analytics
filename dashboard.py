...
with tab1:
    ...
    if city_sel!="All": m = m[m.city==city_sel]
    ...
    if m.empty:
        ...
        avg_d = round((latest.avg_trip_duration*latest.total_trips).sum()/tot,2) if tot else 0
        ...
        st.line_chart(m.set_index("event_date")[["total_trips","avg_trip_duration","cancellations"]])

with tab2:
    a = alerts_df.copy()
    if alert_sel!="All": a = a[a.alert_type==alert_sel]
